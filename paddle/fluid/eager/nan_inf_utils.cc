...
//         api_name, tensor_name, *dense_tensor, place);
    if (paddle::platform::is_gpu_place(place) || paddle::platform::is_xpu_place(place)) {
        phi::DenseTensor cpu_tensor;
        cpu_tensor.Resize(dense_tensor->dims());
        paddle::framework::TensorCopySync(*dense_tensor, paddle::platform::CPUPlace(), &cpu_tensor);
        paddle::framework::details::tensor_check<phi::CPUContext>(api_name, tensor_name, cpu_tensor, place, folder_path);
    } else {
        paddle::framework::details::tensor_check<phi::CPUContext>(api_name, tensor_name, *dense_tensor, place, folder_path);
    }
  }
}
